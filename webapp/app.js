...
setTimeout(() => {
    ...
    hud.style.cssText = `
        position: fixed; bottom: 100px; left: 8px; right: 8px;
        background: rgba(0,0,0,0.9); color: #0f0; padding: 10px;
        font-family: monospace; font-size: 10px; z-index: 99999;
    ...
    hud.innerHTML = `
        <div style="color:#ff0;font-weight:bold;margin-bottom:6px;">🔍 DEBUG HUD</div>
        <div>scrollElement: <b style="color:#fff">${document.scrollingElement === document.body ? 'BODY' : 'HTML'}</b></div>
        <div>body: ${document.body.scrollHeight} / ${document.body.clientHeight}</div>
        <div>html: ${document.documentElement.scrollHeight} / ${document.documentElement.clientHeight}</div>
        <div>#app: ${app ? app.offsetHeight : 'N/A'}</div>
        <div>.screens: ${screens ? screens.offsetHeight + ' / scrollH=' + screens.scrollHeight : 'N/A'}</div>
        <div>.screen.active: ${active ? active.offsetHeight + ' / scrollH=' + active.scrollHeight : 'N/A'}</div>
        <div>window.innerHeight: ${window.innerHeight}</div>
        <div>window.scrollY: ${window.scrollY}</div>
        <div style="margin-top:6px;color:#0ff;">initData: ${(initData||'').length} chars</div>
        <div style="color:#0ff;">tg.version: ${window.Telegram?.WebApp?.version || '?'}</div>
        <div style="color:#0ff;">tg.platform: ${window.Telegram?.WebApp?.platform || '?'}</div>
    `;
    body.appendChild(hud);
}, 2000);

window.addEventListener('scroll', () => {
    const hud = document.querySelector('div[style*="99999"]');
    if (hud) {
        hud.innerHTML = hud.innerHTML.replace(
            /window\.scrollY: \d+/,
